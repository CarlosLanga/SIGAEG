...
if (!is_array($rows)) {
    json_out(['ok' => false, 'message' => 'Dados inválidos.']);
}
...
if (!$resPermissao || $resPermissao->num_rows === 0) {
    json_out(['ok' => false, 'message' => 'Sem permissao para guardar estes resultados.']);
}
$avaliacaoPermissao = $resPermissao->fetch_assoc();
$turmaPermitida = (int)($avaliacaoPermissao['turma_id'] ?? 0);
...
foreach ($rows as $row) {
    $formando_id = (int)($row['formando_id'] ?? 0);
    if ($formando_id <= 0) {
        continue;
    }
    $resFormando = $conn->query("SELECT id FROM formandos WHERE id = $formando_id AND turma_id = $turmaPermitida LIMIT 1");
    if (!$resFormando || $resFormando->num_rows === 0) {
        continue;
    }

    $notaRaw = trim((string)($row['nota'] ?? ''));
    ...
    if ($notaRaw !== '') {
        ...
        if ($notaFloat > 100) $notaFloat = 100;
        $nota = $notaFloat;
        $resultado = $notaFloat >= 80 ? 'A' : 'NA';
    }

    $notaSql = $nota === null ? "NULL" : (float)$nota;
    $observacaoSql = $observacao !== '' ? "'" . $conn->real_escape_string($observacao) . "'" : "NULL";

    $conn->query("
        INSERT INTO avaliacoes_resultados (avaliacao_id, formando_id, nota_obtida, resultado, observacao)
        VALUES ($avaliacao_id, $formando_id, $notaSql, '$resultado', $observacaoSql)
        ON DUPLICATE KEY UPDATE
            nota_obtida = $notaSql,
            resultado = '$resultado',
            observacao = $observacaoSql
    ");
}
