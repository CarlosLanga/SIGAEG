...
$sql = "
    SELECT f.nome_completo, f.codigo_formando, f.sexo, f.data_nascimento, t.nome_turma, c.nome_curso
    FROM formandos f
...
<body>
    <div class="header">
        ...
        <div class="title">Lista de Formandos</div>
...
foreach ($rows as $r) {
    ...
    $html .= '<tr>
        ...
        <td>' . esc($sexo) . '</td>
        <td>' . esc(calc_idade($r['data_nascimento'] ?? null)) . '</td>
        <td></td>
    </tr>';
}
...
$mpdf->WriteHTML($html);
$mpdf->Output('formandos_' . preg_replace('/[^A-Za-z0-9_-]/', '_', $nomeTurma) . '.pdf', 'I');
