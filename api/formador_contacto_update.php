...
if (!isset($_SESSION['usuario_id']) || (int)($_SESSION['nivel_acesso'] ?? 0) !== 2) {
    echo json_encode(['estado' => 'erro', 'message' => 'Sem permissões.']);
    exit;
}

if (!$conn) {
    log_erro('formador_contacto_update', 'Falha de ligação à base de dados');
...
$stmt = $conn->prepare("UPDATE formadores SET telefone = ? WHERE usuario_id = ? LIMIT 1");
$stmt->bind_param("si", $contacto, $usuarioId);
if (!$stmt->execute()) {
    log_erro('formador_contacto_update', $stmt->error);
    $stmt->close();
